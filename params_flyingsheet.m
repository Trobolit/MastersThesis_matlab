...
p.Ts = 1/120; %0.0025; %400 PixHawk rate; %0.05;% 20Hz COde indicated?
% Output log Ts
p.Ts_log = p.Ts; 
...
p.Ix=0.07; % kg*m^2
p.Iy=0.0079*2; % Sheet maybe twice weight of ecosoar?
p.Iz=0.08;
...
%Aileron stuff
p.Sa=0.14*(0.27-0.2); % Area outside wake, one aileron, 7cm width inside wake, 20cm inside.

p.Saw=0.14*0.2; % 80% of propwidth; 
...
%Winglet stuff
p.SW = 0.13*0.15; %0.04; % Guess, winglet area?

% Wing stuff, areas on one wing, not both
%total wing area: 0.67*0.27/2;
p.Sw_w = p.wakewidth*0.39;   % wing area in wake, guess
p.Sw = 0.39*(0.76/2-p.wakewidth);    % wing area outside wake
p.b = 0.76;

%p.c = @(y) 0.28 - (0.28-0.09).*y./(p.b/2); % This should be pretty accurate

p.Cmyl = 0; % Wild guess, need to see what number here gives reasonable pitch down moment with speed. This will be this equations only tuning parameter. "l" is disregarded.
p.Cwy = 1;   % wild guess.
%distances
p.yf = 0.36; 
p.yt = 0.24;
p.xW = 0.28;
p.xf = 0.29;
p.xac = 0.01; % depends on c.g. 1cm ahead is possible
p.zW = 0.13/2;
p.yw = p.wakewidth;
% Integral eval point
